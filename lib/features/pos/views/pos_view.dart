...
class _POSViewState extends ConsumerState<POSView> {
  ...
  Future<void> _handleCheckout(bool printReceipt) async {
    ...
    await showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Customer Details (Optional)'),
          content: Column(
      ...
      if (proceed == true) {
        try {
          await ref.read(posControllerProvider.notifier).checkout(
            ...
            customerName: nameCtrl.text.trim().isNotEmpty ? nameCtrl.text.trim() : null,
            customerPhone: phoneCtrl.text.trim().isNotEmpty ? phoneCtrl.text.trim() : null,
          );
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(printReceipt ? 'Bill printed & saved successfully!' : 'Bill saved successfully!')),
            );
          }
        } catch (e) {
          if (context.mounted) {
            ...
            ElevatedButton(
              onPressed: () async {
                if (formKey.currentState!.validate()) {
                  await ref.read(posControllerProvider.notifier).addNewCustomer(
                        nameCtrl.text.trim(),
                        phoneCtrl.text.trim(),
                        addrCtrl.text.trim(),
                      );
                            ...
                            return Card(
                              ...
                              child: InkWell(
                                onTap: outOfStock
                                    ? null
                                    : () {
                                        ref.read(posControllerProvider.notifier).addToCart(p);
                                        _barcodeFocusNode.requestFocus();
                                      },
                                child: Padding(
                                    ...
                                    children: [
                                               ...
                                               ? ClipRRect(
                                                   ...
                                                   child: Image.network(
                                                     ...
                                                     errorBuilder: (context, error, stackTrace) => Icon(
                                                       Icons.inventory_2,
                                                       size: 40,
                                                       color: outOfStock ? Colors.grey : theme.colorScheme.primary,
                                                     ),
                                                   ),
                                                 )
                                               : Icon(
                                      ...
                                      Text(
                                        ...
                                        style: TextStyle(
                                          ...
                                          color: outOfStock
                                              ? Colors.red
                                              : (p.stock <= p.minimumStock ? Colors.orange : Colors.grey),
                                        ),
                                      ),
                                    ],
                                  ),
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                               title: Column(
                                 crossAxisAlignment: CrossAxisAlignment.start,
                                 children: [
                                   Text(item.product.name, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold)),
                                   const SizedBox(height: 2),
                                   Text(
                               ...
                               subtitle: Text('Rs. ${item.product.retailPrice.toStringAsFixed(0)} each'),
                              trailing: Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  IconButton(
                                    ...
                                    onPressed: () {
                                      ref.read(posControllerProvider.notifier).updateQuantity(item.product.productId, item.quantity - 1);
                                      _barcodeFocusNode.requestFocus();
                                    },
                                  ),
                                  Text(
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () {
                                      ref.read(posControllerProvider.notifier).removeFromCart(item.product.productId);
                                      _barcodeFocusNode.requestFocus();
                                    },
                                  ),
                                ],
                          ...
                          Expanded(
                            child: DropdownButtonFormField<CustomerModel?>(
                              ...
                              items: [
                                ...
                                ...posState.customers.map((c) => DropdownMenuItem(value: c, child: Text('${c.name} (${c.phone ?? "-"})'))),
                              ],
                              onChanged: (val) {
                                ref.read(posControllerProvider.notifier).selectCustomer(val);
                                _barcodeFocusNode.requestFocus();
                              },
                            ),
                          ),
                        ...
                        children: [
                          Expanded(
                            child: DropdownButtonFormField<String>(
                              ...
                              items: ['Cash', 'Card', 'Mobile Payment', 'Credit (Khata)']
                                  .map((m) => DropdownMenuItem(value: m, child: Text(m)))
                                  .toList(),
                              onChanged: (val) {
                                if (val != null) {
                                  ref.read(posControllerProvider.notifier).setPaymentMethod(val);
                                }
                                _barcodeFocusNode.requestFocus();
                              },
                            ),
                          ...
                          Expanded(
                            child: TextFormField(
                              ...
                              onChanged: (val) {
                                ...
                                ref.read(posControllerProvider.notifier).setFlatDiscount(d);
                              },
                            ),
                          ),
                        ],
                          ...
                          Expanded(
                            child: TextFormField(
                              ...
                              onChanged: (val) {
                                ...
                                ref.read(posControllerProvider.notifier).setPaidAmount(p);
                              },
                            ),
                          ),
                          const SizedBox(width: 12),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          const Text('Total Discount:', style: TextStyle(color: Colors.grey)),
                          Text('- Rs. ${posState.totalDiscount.toStringAsFixed(0)}', style: const TextStyle(color: Colors.green)),
                        ],
                      ),
