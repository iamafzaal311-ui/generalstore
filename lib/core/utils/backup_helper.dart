...
class BackupHelper {
  static Future<String> exportDatabaseToJson(LocalDbService db) async {
    ...
    backup['users'] = db.usersBox.values.map((u) => {
      'userId': u.userId,
      'username': u.username,
      'fullName': u.fullName,
    ...
    backup['categories'] = db.categoriesBox.values.map((c) => {
    ...
    backup['brands'] = db.brandsBox.values.map((b) => {
      'brandId': b.brandId,
      'name': b.name,
    ...
    backup['suppliers'] = db.suppliersBox.values.map((s) => {
    ...
    backup['customers'] = db.customersBox.values.map((c) => {
    ...
    backup['products'] = db.productsBox.values.map((p) => {
    ...
    backup['sales'] = db.salesBox.values.map((s) => {
      'saleId': s.saleId,
  ...
  static Future<void> restoreDatabaseFromJson(LocalDbService db, String jsonContent) async {
